    ...
    if (event.type == YAML_SCALAR_EVENT) {
      if (level == 2) {
        ...
        if (previous_level_number != -1 && value_count > 0) {
          add_level(levels, previous_level_number, values, value_lengths,
                    value_count);
          values = NULL;
          value_lengths = NULL;
          value_count = 0;
        ...
        int *sequence = parse_int_sequence(
            (const char *)event.data.scalar.value, &sequence_length);
        value_count++;
        ...
        //				printf("Parsed sequence for level %d: ",
        //current_level_number); 				for (size_t i = 0; i < sequence_length; i++) {
        //					printf("%d ", sequence[i]);
        //				}
        //				printf("\n");
      }
...
void play_game(CardinalChainsGame *game, Levels *levels, int grid_rows,
  ...
  //	printf("Before move_chain:
  //game->chains[current_chain][game->chain_lengths[current_chain] - 1].x:
  //%d\n", 		   game->chains[current_chain][game->chain_lengths[current_chain] - 1].x
  //+ 1); 	printf("Before move_chain:
  //game->chains[current_chain][game->chain_lengths[current_chain] - 1].y:
  //%d\n", 		   game->chains[current_chain][game->chain_lengths[current_chain] - 1].y
  //+ 1);
  ...
  while (true) {
    ...
    //		if (moved) {
    //			printf("After move_chain:
    //game->chains[current_chain][game->chain_lengths[current_chain] - 1].x:
    //%d\n", 				   game->chains[current_chain][game->chain_lengths[current_chain] -
    //1].x); 			printf("After move_chain:
    //game->chains[current_chain][game->chain_lengths[current_chain] - 1].y:
    //%d\n", 				   game->chains[current_chain][game->chain_lengths[current_chain] -
    //1].y);
    //		}
